...
int sa_init_sc(struct sa_ctx_info *ctx, const u8 *enc_key,
	       ...
	       const struct sa_eng_cfg *eng_cfg, u8 enc,
	       u32 *swinfo)
{
	int use_enc = 0;
	...
	if (eng_cfg->auth_eng_id <= SA_ENG_ID_EM2)
		use_enc = 1;

	/* Determine the order of encryption & Authentication contexts */
	if (enc || !use_enc) {
		if (eng_cfg->aalg_id == SA_AALG_ID_GMAC) {
			eng0_f = SA_CTX_SIZE_TO_DMA_SIZE(eng_cfg->auth_sc_size);
			eng1_f = SA_CTX_SIZE_TO_DMA_SIZE(eng_cfg->en_sc_size);
		} else {
			eng0_f = SA_CTX_SIZE_TO_DMA_SIZE(eng_cfg->en_sc_size);
			eng1_f = SA_CTX_SIZE_TO_DMA_SIZE(eng_cfg->auth_sc_size);
		}
		enc_sc_offset = SA_CTX_PHP_PE_CTX_SZ;
		auth_sc_offset = enc_sc_offset + eng_cfg->en_sc_size;
	} else {
		eng0_f = SA_CTX_SIZE_TO_DMA_SIZE(eng_cfg->auth_sc_size);
		eng1_f = SA_CTX_SIZE_TO_DMA_SIZE(eng_cfg->en_sc_size);
		auth_sc_offset = SA_CTX_PHP_PE_CTX_SZ;
		enc_sc_offset = auth_sc_offset + eng_cfg->auth_sc_size;
	}
	...
	if (eng_cfg->en_sc_size) {
		ret = sa_set_sc_enc(eng_cfg->ealg_id, enc_key, enc_key_sz, aad_len,
	...
	if (eng_cfg->auth_sc_size) {
		if (use_enc) {
			if (sa_set_sc_enc(eng_cfg->aalg_id, auth_key, auth_key_sz,
					  aad_len, 0, &sc_buf[auth_sc_offset]))
				return -1;
		} else
			sa_set_sc_auth(eng_cfg->aalg_id, auth_key, auth_key_sz,
	...
	/* Setup SWINFO */
	if (eng_cfg->ealg_id == SA_EALG_ID_GCM) {
		/* For GCM enc and dec performed by same engine */
		first_engine = eng_cfg->en_eng_id;
	} else if ((eng_cfg->ealg_id == SA_EALG_ID_NULL) ||
		   (eng_cfg->ealg_id == SA_EALG_ID_NONE))
		first_engine = eng_cfg->auth_eng_id;
	else
		first_engine = enc ? eng_cfg->en_eng_id : eng_cfg->auth_eng_id;

	hash_size = AES_BLOCK_SIZE;
	if (eng_cfg->aalg_id != SA_AALG_ID_NONE) {
		hash_size = sa_get_hash_size(eng_cfg->aalg_id);
		if (!hash_size)
			return -EINVAL;
	}
...
static int sa_aead_setkey(struct crypto_aead *authenc, const u8 *key,
			  unsigned int keylen,
			  const struct sa_eng_cfg *eng_cfg)
{
	...
	cfg.enc1st = 1;
	cfg.aalg = eng_cfg->aalg_id;
	cfg.enc_eng_id = eng_cfg->en_eng_id;
	cfg.auth_eng_id = eng_cfg->auth_eng_id;
	...
	if (sa_init_sc(&ctx->enc, keys.enckey, keys.enckeylen,
		       ...
		       eng_cfg, 1, &ctx->enc.epib[1]))
		goto badkey;

	cmdl_len = sa_format_cmdl_gen(&cfg, (u8 *)ctx->enc.cmdl,
			      ...
			      struct sa_eng_cfg *eng_cfg)
{
	...
	if (eng_cfg->ealg_id != SA_EALG_ID_NONE) {
		/*  GCM  */
		enckey_len = keylen - 4;
		enc_key = key;
	...
	if (sa_init_sc(&ctx->enc, enc_key, enckey_len, cfg.akey,
		       cfg.akey_len, eng_cfg, 1, &ctx->enc.epib[1]))
		goto badkey;
	...
	if (sa_init_sc(&ctx->dec, enc_key, enckey_len, cfg.akey,
		       cfg.akey_len, eng_cfg, 0, &ctx->dec.epib[1]))
		goto badkey;
...
struct sa_eng_cfg sa_hmac_sha1_cbc_aes_cfg = {
	"authenc(hmac(sha1),cbc(aes))",
	SA_EALG_ID_AES_CBC, SA_ENG_ID_EM1, SA_CTX_ENC_TYPE1_SZ,
	SA_AALG_ID_HMAC_SHA1, SA_ENG_ID_AM1, SA_CTX_AUTH_TYPE2_SZ
};

static int sa_hmac_sha1_cbc_aes_setkey(struct crypto_aead *authenc,
				       const u8 *key, unsigned int keylen)
{
	return sa_aead_setkey(authenc, key, keylen, &sa_hmac_sha1_cbc_aes_cfg);
}

struct sa_eng_cfg sa_hmac_sha1_cbc_des3_ede_cfg = {
	"authenc(hmac(sha1),cbc(des3_ede))",
	SA_EALG_ID_3DES_CBC, SA_ENG_ID_EM1, SA_CTX_ENC_TYPE1_SZ,
	SA_AALG_ID_HMAC_SHA1, SA_ENG_ID_AM1, SA_CTX_AUTH_TYPE2_SZ
};

static int sa_hmac_sha1_cbc_des3_ede_setkey(struct crypto_aead *authenc,
					    const u8 *key, unsigned int keylen)
{
	return sa_aead_setkey(authenc, key, keylen,
			      &sa_hmac_sha1_cbc_des3_ede_cfg);
}

struct sa_eng_cfg sa_xcbc_aes_cbc_aes_cfg = {
	"authenc(xcbc(aes),cbc(aes))",
	SA_EALG_ID_AES_CBC, SA_ENG_ID_EM1, SA_CTX_ENC_TYPE1_SZ,
	SA_AALG_ID_AES_XCBC, SA_ENG_ID_EM1, SA_CTX_AUTH_TYPE1_SZ
};

static int sa_xcbc_aes_cbc_aes_setkey(struct crypto_aead *authenc,
				      const u8 *key, unsigned int keylen)
{
	return sa_aead_setkey(authenc, key, keylen, &sa_xcbc_aes_cbc_aes_cfg);
}

struct sa_eng_cfg sa_xcbc_aes_cbc_des3_ede_cfg = {
...
static int
sa_xcbc_aes_cbc_des3_ede_setkey(struct crypto_aead *authenc, const u8 *key,
				unsigned int keylen)
{
	return sa_aead_setkey(authenc, key, keylen,
			      &sa_xcbc_aes_cbc_des3_ede_cfg);
}

struct sa_eng_cfg sa_hmac_sha1_ecb_null_cfg = {
...
static int
sa_hmac_sha1_ecb_null_setkey(struct crypto_aead *authenc, const u8 *key,
			     unsigned int keylen)
{
	return sa_aead_setkey(authenc, key, keylen,
			      &sa_hmac_sha1_ecb_null_cfg);
}

struct sa_eng_cfg sa_rfc4106_gcm_aes_cfg = {
	"rfc4106(gcm(aes))",
	SA_EALG_ID_GCM, SA_ENG_ID_EM1, SA_CTX_ENC_TYPE2_SZ,
	SA_AALG_ID_NONE, SA_ENG_ID_NONE, 0
};

static int
sa_rfc4106_gcm_aes_setkey(struct crypto_aead *authenc, const u8 *key,
			  unsigned int keylen)
{
	return sa_aead_gcm_setkey(authenc, key, keylen,
				  &sa_rfc4106_gcm_aes_cfg);
}

struct sa_eng_cfg sa_rfc4543_gcm_aes_cfg = {
	"rfc4543(gcm(aes))",
...
static int
sa_rfc4543_gcm_aes_setkey(struct crypto_aead *authenc, const u8 *key,
			  unsigned int keylen)
{
	return sa_aead_gcm_setkey(authenc, key, keylen,
				  &sa_rfc4543_gcm_aes_cfg);
}

struct sa_eng_cfg sa_hmac_sha256_cbc_aes_cfg = {
...
static int
sa_hmac_sha256_cbc_aes_setkey(struct crypto_aead *authenc, const u8 *key,
			      unsigned int keylen)
{
	return sa_aead_setkey(authenc, key, keylen,
			      &sa_hmac_sha256_cbc_aes_cfg);
}
...
static struct sa_alg_tmpl sa_algs[] = {
	...
	{	.type = CRYPTO_ALG_TYPE_AEAD,
		.alg.aead = {
			...
			.setkey	= sa_hmac_sha1_cbc_aes_setkey,
	...
	{	.type = CRYPTO_ALG_TYPE_AEAD,
		.alg.aead = {
			...
			.init = sa_cra_init_aead,
			.exit = sa_exit_tfm_aead,
			.setkey	= sa_hmac_sha1_cbc_des3_ede_setkey,
			.encrypt = sa_aead_encrypt,
			.decrypt = sa_aead_decrypt,
		}
	},
	{       .type = CRYPTO_ALG_TYPE_AEAD,
		.alg.aead = {
			...
			.init = sa_cra_init_aead,
			.exit = sa_exit_tfm_aead,
			.setkey	= sa_xcbc_aes_cbc_aes_setkey,
			.encrypt = sa_aead_encrypt,
			.decrypt = sa_aead_decrypt,
		}
	},
	{	.type = CRYPTO_ALG_TYPE_AEAD,
		.alg.aead = {
			...
			.init = sa_cra_init_aead,
			.exit = sa_exit_tfm_aead,
			.setkey	= sa_xcbc_aes_cbc_des3_ede_setkey,
			.encrypt = sa_aead_encrypt,
			.decrypt = sa_aead_decrypt,
		}
	},
	{	.type = CRYPTO_ALG_TYPE_AEAD,
		.alg.aead = {
			...
			.init = sa_cra_init_aead,
			.exit = sa_exit_tfm_aead,
			.setkey	= sa_hmac_sha1_ecb_null_setkey,
			.encrypt = sa_aead_encrypt,
			.decrypt = sa_aead_decrypt,
		}
	},
	{	.type = CRYPTO_ALG_TYPE_AEAD,
		.alg.aead = {
			...
			.init = sa_cra_init_aead,
			.exit = sa_exit_tfm_aead,
			.setkey	= sa_rfc4106_gcm_aes_setkey,
			.encrypt = sa_aead_encrypt,
			.decrypt = sa_aead_decrypt,
		}
	},
	{	.type = CRYPTO_ALG_TYPE_AEAD,
		.alg.aead = {
			...
			.init = sa_cra_init_aead,
			.exit = sa_exit_tfm_aead,
			.setkey	= sa_rfc4543_gcm_aes_setkey,
			.encrypt = sa_aead_encrypt,
			.decrypt = sa_aead_decrypt,
		}
	},
	{	.type = CRYPTO_ALG_TYPE_AEAD,
		.alg.aead = {
			...
			.init = sa_cra_init_aead,
			.exit = sa_exit_tfm_aead,
			.setkey	= sa_hmac_sha256_cbc_aes_setkey,
			.encrypt = sa_aead_encrypt,
			.decrypt = sa_aead_decrypt,
		}
	},
