...
#define CRA_NAME_MAX	64
struct sa_eng_cfg {
	char	cra_name[CRA_NAME_MAX];
	int	ealg_id;
	int	en_eng_id;
	int	en_sc_size;
	int	aalg_id;
				     ...
				     u16 key_sz)
{
	...
	rounds = 6 + ctx.key_length / 4;

	if (sub_key1) {
		memset(sub_key1, 0x01, AES_BLOCK_SIZE);
		__aes_arm_encrypt(ctx.key_enc, rounds, sub_key1, sub_key1);
	}

	if (sub_key2) {
		memset(sub_key2, 0x02, AES_BLOCK_SIZE);
		__aes_arm_encrypt(ctx.key_enc, rounds, sub_key2, sub_key2);
	}

	if (sub_key3) {
		memset(sub_key3, 0x03, AES_BLOCK_SIZE);
		__aes_arm_encrypt(ctx.key_enc, rounds, sub_key3, sub_key3);
	}
